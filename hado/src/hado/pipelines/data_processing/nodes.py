...
def clean_and_classify_barthel(
    df: pd.DataFrame,
...
def clean_ps_ecog(
    ...
    barthel_col: str = "barthel_classification",
) -> pd.DataFrame:
    """
...
def clean_and_classify_gds_fast(
    df: pd.DataFrame,
    ...
    for classification, value in barthel_to_gds_mappings.items():
        df.loc[
            (df["gds_fast"] == "si") & (df[barthel_col] == classification), "gds_fast"
        ] = value
    ...
    df["gds_fast"] = (
        ...
        .astype(str)
        .apply(
            lambda value: str(max([int(digit) for digit in value]))
            if value.isdigit()
            else value
        )
    )
...
def replace_words(
    ...
    replacement_dict_transfusion: Dict,
    ...
    replacement_dict_ap : Dict
        Dictionary with the words to replace and the corresponding replacements in the 'ap' column.
    ...
    cols_dicts = [
        ...
        ("transfusion", replacement_dict_transfusion),
...
def add_location_and_map_names(
    ...
    df["latitude"] = df["ayuntamiento"].map(
        lambda x: locations.get(x, {}).get("lat", "desconocido")
    )
    df["longitude"] = df["ayuntamiento"].map(
        lambda x: locations.get(x, {}).get("lon", "desconocido")
    )
...
def apply_categorization(df: pd.DataFrame, parameters) -> pd.DataFrame:
    ...
    df["hospital_category"] = df["h_procedencia"].apply(
        lambda x: classify(x, parameters["hospital_categories"])
    )
    df["diagnosis_category"] = df["diagnostico"].apply(
        lambda x: classify(x, parameters["diagnosis_categories"])
    )
    df["ingreso_category"] = df["motivo_ing"].apply(
        lambda x: classify(x, parameters["ingreso_categories"])
    )
    df["alta_category"] = df["motivo_alta"].apply(
        lambda x: classify(x, parameters["alta_categories"])
    )
    df["procedencia_category"] = df["s_procedencia"].apply(
        lambda x: classify(x, parameters["procedencia_categories"])
    )
    df["otros_category"] = df["otros"].apply(
        lambda x: classify(x, parameters["otros_categories"])
    )
...
def categorize_and_group_combined_otros(df: pd.DataFrame, parameters) -> pd.DataFrame:
    ...
    df["categorized_combined_otros"] = df["combined_otros"].apply(
        classify_combined_otros
    )
    ...
    df["categorized_combined_otros"] = df["categorized_combined_otros"].apply(
        lambda x: x if x in main_categories else "Otros"
    )
...
def encoding_variables(df: pd.DataFrame, parameters) -> pd.DataFrame:
    ...
    def apply_frequency_encoding(column_name):
        ...
        df[column_name] = (
            df[column_name].map(frequencies).fillna(0)
    ...
    hospital_category_map = parameters[
        "hospital_category_map"
    ]  # Values {Santiago, no, Otros}
    ...
    df["procedencia_category"] = df["procedencia_category"].map(
        procedencia_category_map
    )
    df["otros_category"] = df["otros_category"].map(otros_category_map)
    df["categorized_combined_otros"] = df["categorized_combined_otros"].map(
