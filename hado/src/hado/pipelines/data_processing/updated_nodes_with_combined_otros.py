...
def encoding_variables(df):
    ...
    def map_source_hospital(hospital_name):
    ...
    df['source_hospital'] = df['h_procedencia'].apply(map_source_hospital)
    df['source_hospital'].replace({'Santiago': SANTIAGO, 'no': NO, 'otros': OTHERS}, inplace=True)
    ...
    # Columns order
    columns_order = ['h_procedencia','source_hospital', 's_procedencia', 'diagnostico', 'motivo_ing', 'motivo_alta', 'fecha_alta',
...
def apply_categorization(data: pd.DataFrame) -> pd.DataFrame:
    data['diagnosis_category'] = data['cancer orl'].apply(classify_diagnosis)
    data['ingreso_category'] = data['control sintomas'].apply(classify_ingreso)
    data['alta_category'] = data['motivo_alta'].apply(classify_alta)
    data['procedencia_category'] = data['s_procedencia'].apply(classify_procedencia)
    data['otros_category'] = data['otros'].apply(classify_otros)
    return data
