    ...
    try:
        if key == "hado_17":
            if "unnamed.1" in df.columns:
                df = df.drop(
                    columns="unnamed.1"
                )  # Remove the column 'unnamed.1' from the DataFrame 'hado_17'
    except Exception as e:
    ...
    try:
        if key in ["hado_17", "hado_18", "hado_19"]:
            if set(["astenia", "anorexia"]).issubset(df.columns):
                df["ast_anorx"] = np.where(
                    df["astenia"] == "si", "si", df["anorexia"]
                )  # Combine the columns 'astenia' and 'anorexia'
...
def clean_and_sort_column_names(dfs):
    ...
    for key in dfs:
        try:
            if re.match(
                r"hado_\d+", key
            ):  # check if the key matches the pattern 'hado_' followed by one or more digits
                df = dfs[key]
                ...
                dfs[
                    key
                ] = df  # replace the original DataFrame in the dictionary with the cleaned DataFrame
        except Exception as e:
...
def rename_all_columns(
    dataframes: Dict[str, pd.DataFrame], column_mappings: Dict[str, Dict[str, str]]
    ...
    for key, df in dataframes.items():
        try:
            ...
            print(
                f"Error renaming columns in {key} with provided column mappings: {str(e)}"
            )
    return dataframes
